...
(() => {
  ...
  const slideCount = typeof SLIDE_COUNT !== "undefined" ? SLIDE_COUNT : 54;
  const slideTitles = typeof SLIDE_TITLES !== "undefined" ? SLIDE_TITLES : [];
  const assetVersion = typeof SLIDE_ASSET_VERSION !== "undefined" ? SLIDE_ASSET_VERSION : "2026-05-15-t12b";
  ...
  function createSimulatorOverlay(interaction) {
    ...
    iframe.title = `Laboratorio: ${interaction.title}`;
    iframe.loading = "lazy";
    ...
    message.textContent = "Si el laboratorio no carga, usa el botón para abrirlo en navegador.";
    ...
    addDebugChrome(overlay, `laboratorio · ${interaction.title}`, interaction);
    return overlay;
  }
  ...
  function createPollOverlay(poll) {
    ...
    addDebugChrome(overlay, `encuesta · ${poll.title}`, poll);
    return overlay;
  }
  ...
  function renderDebug(activeInteractions) {
    ...
    const slideNumber = current + 1;
    const inactivePolls = Object.values(pollsConfig).filter((poll) => !poll.displayUrl);
    const rows = activeInteractions.map((item) => {
      const url = item.embedUrl || item.displayUrl || item.openUrl || "";
      return `<li><code>${item.type}</code> ${escapeHtml(item.title || item.key)}<br>x=${item.x}, y=${item.y}, w=${item.w}, h=${item.h}<br>${escapeHtml(url || "fallback de aula virtual")}</li>`;
    }).join("");
    debugPanel.innerHTML = `
      ...
      <p><strong>Encuestas en fallback:</strong> ${inactivePolls.length}</p>
    `;
  }
